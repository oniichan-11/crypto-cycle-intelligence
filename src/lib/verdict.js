...
export function verdict(s){
  if(!s.fg||!s.dom) return{phase:"SYNCING DATA",col:"#6B7484",risk:"—",desc:"Connecting...",score:0,active:0};
  ...
  if(ps>=2.5)return{phase:"DEEP ACCUMULATION",col:"#17A257",risk:"AGGRESSIVE LONG",desc:`Maximum on-chain conviction. ${dCtx}. Stage entries BTC → ETH → L1s.`,score,active:n};
  if(ps>=1.2)return{phase:"EARLY BULL",col:"#2E9E5B",risk:"RISK-ON",desc:`On-chain bullish, sentiment recovering. ${dCtx}. Full risk-on across core sectors.`,score,active:n};
  if(ps>=0.2)return{phase:"MID BULL",col:"#8A6100",risk:"SELECTIVE",desc:`Expansion underway. ${dCtx}. Hold core, selective rotation, watch NUPL & funding.`,score,active:n};
  if(ps>=-1)return{phase:"LATE BULL / CAUTION",col:"#D9720F",risk:"SCALE OUT",desc:`Euphoria building. ${dCtx}. Systematic profit-taking, cut leverage.`,score,active:n};
  return{phase:"DISTRIBUTION / BEAR",col:"#E23A4E",risk:"DEFENSIVE",desc:`On-chain distribution. ${dCtx}. Capital preservation — stables/cash.`,score,active:n};
}
