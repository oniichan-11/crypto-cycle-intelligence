...
export default function RobinhoodEco({ sector, defi, rhFees }) {
  ...
  return (
    <>
      <div style={{padding:"10px 14px",background:"#FDEEEE",borderRadius:8,border:"1px solid #E23A4E44",marginBottom:10}}>
        <span style={{color:"#D9720F",fontSize:12,lineHeight:1.7}}>
          <span style={{color:"#E23A4E",fontWeight:700}}>RESEARCH ONLY — NOT INVESTMENT ADVICE.</span> This panel tracks publicly disclosed economic links to Robinhood Chain. It makes no entry, exit, valuation, or pairing recommendations. Tokenized stocks are derivative claims issued by third parties — not shares with ownership rights — and can trade away from the underlying, especially outside market hours.
        </span>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:10}}>
        <Card title="RH Chain TVL — DefiLlama">
          <div style={{fontSize:26,fontWeight:700,color:"#17A257",fontFamily:"inherit"}}>{chain?fmtBig(chain.tvl):"—"}</div>
          <div style={{color:"#6B7686",fontSize:11,marginTop:4}}>Mainnet 2026-07-01 · Arbitrum stack · no native token</div>
        </Card>
        <Card title="Arbitrum TVL — Comparison">
          <div style={{fontSize:26,fontWeight:700,color:"#171B24",fontFamily:"inherit"}}>{arb?fmtBig(arb.tvl):"—"}</div>
          <div style={{color:"#6B7686",fontSize:11,marginTop:4}}>Parent stack — collects 10% of RH Chain fees</div>
        </Card>
        <Card title="All-App Fees 24h">
          <div style={{fontSize:26,fontWeight:700,color:"#F7931A",fontFamily:"inherit"}}>{rhFees?fmtBig(rhFees.total24h):"—"}</div>
          <div style={{color:"#6B7686",fontSize:11,marginTop:4}}>Sum of every protocol's fees on the chain — higher than "chain fee" headlines</div>
        </Card>
        <Card title="All-App Fees 7d">
          <div style={{fontSize:26,fontWeight:700,color:"#F7931A",fontFamily:"inherit"}}>{rhFees?.total7d!=null?fmtBig(rhFees.total7d):"—"}</div>
          <div style={{color:"#6B7686",fontSize:11,marginTop:4}}>DefiLlama aggregate, trailing 7 days</div>
        </Card>
      </div>

      {/* Ecosystem watchlist */}
      <Card title="Disclosed-Exposure Watchlist — CoinGecko (live)" style={{marginBottom:10}}>
        <div style={{display:"grid",gridTemplateColumns:"70px 110px 64px 64px 90px 1fr",gap:"0 8px",padding:"0 4px 6px",borderBottom:"1px solid #E4E8F0"}}>
          {["ASSET","PRICE","24H","7D","MKT CAP","ECONOMIC LINK TO RH CHAIN"].map(h=><div key={h} style={{color:"#6B7686",fontSize:11,letterSpacing:1.5}}>{h}</div>)}
        </div>
        {ECO.map(e=>{
          const c=byId(e.id), p24=c?.price_change_percentage_24h, p7=c?.price_change_percentage_7d_in_currency;
          return (
            <div key={e.id} style={{display:"grid",gridTemplateColumns:"70px 110px 64px 64px 90px 1fr",gap:"0 8px",padding:"8px 4px",borderBottom:"1px solid #EDF0F5",alignItems:"center"}}>
              <div><span style={{background:"#E4E8F0",color:"#F7931A",padding:"2px 5px",borderRadius:3,fontSize:13,fontWeight:700}}>{e.sym}</span></div>
              <div style={{color:"#171B24",fontSize:14,fontWeight:700,fontFamily:"inherit"}}>{c?fmtP(c.current_price):"—"}</div>
              <div style={{color:pCol(p24),fontSize:13,fontWeight:700}}>{fmtPct(p24)}</div>
              <div style={{color:pCol(p7),fontSize:13,fontWeight:700}}>{fmtPct(p7)}</div>
              <div style={{color:"#55606E",fontSize:12}}>{c?fmtBig(c.market_cap):"—"}</div>
              <div style={{color:"#55606E",fontSize:11,lineHeight:1.5}}>{e.role}</div>
            </div>
          );
        })}
        <div style={{marginTop:8,padding:"6px 8px",background:"#F3F5F9",borderRadius:8,fontSize:11,color:"#6B7686",lineHeight:1.6}}>
      ...
      <Card title="Pairs Trading — How It Works (Education, Not Picks)">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[
            ...
            <div key={x.h} style={{background:"#F3F5F9",borderRadius:5,padding:"10px 12px"}}>
              <div style={{color:"#F7931A",fontSize:12,fontWeight:700,letterSpacing:1.5,marginBottom:6}}>{x.h}</div>
              <div style={{color:"#55606E",fontSize:12,lineHeight:1.7}}>{x.t}</div>
            </div>
          ))}
        </div>
        <div style={{marginTop:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{color:"#6B7686",fontSize:11}}>This dashboard surfaces evidence; sizing and positioning decisions are yours. Nothing here is a recommendation.</span>
          <Badge label="EDUCATIONAL" col="#2E9E5B"/>
        </div>
      </Card>
    </>
  );
