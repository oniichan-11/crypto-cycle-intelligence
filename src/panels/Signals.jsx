    ...
    <Card title="Full Confluence Matrix — All Free Sources">
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        {[
          ...
          {group:"Tier 2 — Momentum",col:"#7DEFA1",sigs:[
          ...
          {group:"Tier 3 — Sentiment & Leverage",col:"#FFB800",sigs:[
          ...
          {group:"Macro Overlay",col:"#9BB8D8",sigs:[
        ...
        ].map(group=><div key={group.group} style={{background:"#080E1C",borderRadius:6,padding:"12px 14px"}}><div style={{color:group.col,fontSize:8,fontWeight:700,letterSpacing:1.5,marginBottom:10,borderBottom:`1px solid ${group.col}22`,paddingBottom:6}}>{group.group.toUpperCase()}</div>{group.sigs.map(s=><div key={s.n} style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #10182810"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}><div><div style={{color:"#7A98B8",fontSize:9}}>{s.n}</div><div style={{color:"#2E4060",fontSize:7,marginTop:1}}>{s.src} · FREE</div></div><span style={{color:"#9BB8D8",fontSize:11,fontWeight:700,fontFamily:"monospace"}}>{s.v}</span></div><Badge label={s.s}/></div>)}</div>)}
      </div>
      <div style={{padding:"12px 16px",background:"#080E1C",borderRadius:6,border:`1px solid ${verd.col}33`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{color:"#2E4060",fontSize:7,marginBottom:3}}>SYNTHESIZED VERDICT · {verd.active} ACTIVE SIGNALS · 100% FREE STACK</div><div style={{color:verd.col,fontSize:16,fontWeight:700}}>{verd.phase}</div></div>
